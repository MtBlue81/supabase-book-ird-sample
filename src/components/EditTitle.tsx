...
export const EditTitle = (props: Props) => {
  ...
  const [author, setAuthor] = useState(props.title.author);
  ...
  const handleRemove = useCallback(async () => {
    let { error } = await client.from('manga_subtitle').delete().eq('title_id', props.title.id);
    if (error) {
      alert('Failed: Remove Title.');
    }
    ({ error } = await client.from('manga_title').delete().eq('id', props.title.id));
    if (error) {
      alert('Failed: Remove Title.');
    }
    router.push('/');
  }, [props.title.id, router]);
            ...
            >
              <div className='inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform border border-gray-300 shadow-xl bg-gray-50 rounded-xl'>
                    ...
                    {({ open }) => (
                      <>
                        ...
                        <Disclosure.Panel className='p-4 text-gray-500 text-md'>
                          <Button block onClick={handleRemove} icon={<IconTrash2/>}>REMOVE</Button>
                        </Disclosure.Panel>
                      </>
                    )}
                ...
                <div className='flex justify-center mt-4'>
                  ...
                  <div className='w-32 p-2'>
                    <Button block size='large' icon={<IconSave/>} onClick={handleSave}>Save</Button>
                  </div>
                </div>
              </div>
            </Transition.Child>
